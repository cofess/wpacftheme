...
add_shortcode('youku', function( $atts, $content='') {
	...
	if(preg_match('#http://v.youku.com/v_show/id_(.*?).html#i',$content,$matches)){
		return '<iframe class="video_frame" height='.esc_attr($height).' width='.esc_attr($width).' src="http://player.youku.com/embed/'.esc_attr($matches[1]).'" frameborder=0 allowfullscreen></iframe>';
	}
});
...
add_shortcode('qqv',  function($atts, $content='') {
	...
	if(preg_match('#//v.qq.com/iframe/player.html\?vid=(.+)#i',$content,$matches)){
		...
		return '<iframe class="video_frame" height='.esc_attr($height).' width='.esc_attr($width).' src="http://v.qq.com/iframe/player.html?vid='.esc_attr($matches[1]).'" frameborder=0 allowfullscreen></iframe>';
...
add_shortcode('tudou', function($atts, $content=''){
	...
	if(preg_match('#http://www.tudou.com/programs/view/(.*?)#i',$content, $matches)){
		return '<iframe class="video_frame" width='. esc_attr($width) .' height='. esc_attr($height) .' src="http://www.tudou.com/programs/view/html5embed.action?code='. esc_attr($matches[1]) .'" frameborder=0 allowfullscreen></iframe>';
	}
});
...
add_shortcode('sohutv', function($atts, $content=''){
	...
	if(preg_match('#http://tv.sohu.com/upload/static/share/share_play.html\#(.+)#i',$content,$matches)){
		//var_dump($matches);exit();
		return '<iframe class="video_frame" height='.esc_attr($height).' width='.esc_attr($width).' src="http://tv.sohu.com/upload/static/share/share_play.html#'.esc_attr($matches[1]).'" frameborder=0 allowfullscreen></iframe>';
	}
});

/**
 * 隐藏文章的部分内容，需要注册登录网站后可见
 * https://blog.csdn.net/vermont_/article/details/86026776
 */
