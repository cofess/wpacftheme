...
class Crontrol {
	...
	public function admin_options_page() {
		...
		?>
		<div class="wrap">
		<h2 class="title"><?php esc_html_e( 'WP-Cron Schedules', 'wp-crontrol' ); ?></h2>
		<p style="margin: 0"><?php esc_html_e( 'WP-Cron schedules are the time intervals that are available for scheduling events. You can only delete custom schedules.', 'wp-crontrol' ); ?></p>
		...
		<div class="wrap narrow">
			...
			<p style="margin: 0"><?php esc_html_e( 'Adding a new cron schedule will allow you to schedule events that re-occur at the given interval.', 'wp-crontrol' ); ?></p>
	...
	public function admin_manage_page() {
		...
		<div class="wrap">
		<h1>
			<?php esc_html_e( 'WP-Cron Events', 'wp-crontrol' ); ?>
			<?php
				printf(
					'<a class="button thickbox" href="%s" title="%s">%s</a>',
					esc_url( admin_url( '#TB_inline?width=600&height=600&inlineId=add-cron-schedule-form' ) ),
					esc_html__( 'Add Cron Schedule', 'wp-crontrol' ),
					esc_html__( 'Add Cron Schedule', 'wp-crontrol' )
				);
			?>
		</h1>
			...
			foreach ( $events as $id => $event ) {
				...
				if ( 'crontrol_cron_job' === $event->hook ) {
					...
					foreach ( $this->get_action_callbacks( $event->hook ) as $callback ) {
						$callbacks[] = '<pre>' . self::output_callback( $callback ) . '</pre>';
					}
		...
		<div id="add-cron-schedule-form" style="display: none">
			<?php $this->admin_options_page();?>
		</div>
		</div>
		<?php
	}
