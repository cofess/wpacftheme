...
class WP_SCSS_Compiler {
	...
	private function get_output_path( $handle ) {
		...
		// $dir        = apply_filters( 'wp_scss_cache_path', path_join( $upload_dir['basedir'], 'scss-cache' ) );
		$dir        = apply_filters( 'wp_scss_cache_path', WP_CONTENT_DIR . '/cache/css/' );
	...
	private function get_output_url( $handle ) {
		$upload_dir      = wp_upload_dir();
		// $dir             = apply_filters( 'wp_scss_cache_url', path_join( $upload_dir['baseurl'], 'scss-cache' ) );
		$dir             = apply_filters( 'wp_scss_cache_url', WP_CONTENT_URL .'/cache/css/' );
		$handle_filename = sanitize_key( basename( $handle ) ) . '.css';
