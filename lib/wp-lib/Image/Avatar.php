...
class Avatar
{
    ...
    function __construct($Char, $AvatarSize = 256)
    {
        ...
        $this->LastChar        = mb_strtoupper(substr($Char, -1), "UTF-8");
        $this->AvatarSize      = $AvatarSize;
        $this->Padding         = 30 * ($this->AvatarSize / 256);
        $this->LetterFont      = get_template_directory() . '/static/public/fonts/RobotoMono-Light.ttf';
        $this->AsianFont       = get_template_directory() . '/static/public/fonts/FZHTK.TTF';
    ...
    private function Initialize()
    {
        ...
        $BackgroundColorIndex = (ord($this->Char) + ord($this->LastChar)) % count($MaterialDesignColor);
        ...
        if ($this->IsNotLetter) {
            ...
            $Y        = $Height - $Padding - (2 / 196) * $FontSize;
        }
        // 在圆正中央填入字符
