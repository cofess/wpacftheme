...
namespace Lib\Tencent;
use Lib\Util\Help;
use Lib\Http\Request;

// wp_cache_add_global_groups(array('wechat'));
...
class WeChat{
	...
	public function add_material($media, $type='image', $args=array()){
		...
		if($description){
			$body['description']= Help::json_encode($description);
		}
	...
	public function send_custom_message($touser, $content='', $msgtype='text', $kf_account=''){
	...
	private function http_request($url, $args=array()){
		...
		$response =  Request::http_request($url, $args);
		...
		if(is_wp_error($response)){
			...
			if($errcode == '40001' || $errcode == '40014' || $errcode == '42001'){
				...
				wp_cache_delete($this->appid.'_access_token', 'weixin');
			}elseif($errcode == '50002'){	
				// 50002 用户受限，可能是违规后接口被封禁
				// $robot_option = get_option('weixin-robot');
