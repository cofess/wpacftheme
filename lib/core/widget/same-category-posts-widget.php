...
class Same_Category_Posts_Widget extends \TypeRocket\Register\BaseWidget {

    /**
     * Sets up the widgets name etc
     */
    public function __construct() {
        parent::__construct( 'tr_same_category_posts', __('同分类文章','BT_TEXTDOMAIN'), [
            'classname' => 'Same_Category_Posts_Widget',
            'description' => __('调用相同分类的文章','BT_TEXTDOMAIN')
        ] );
    }

    public function backend($fields) {
        echo $this->form->text(__('Title','BT_TEXTDOMAIN'))->setName('title');
        echo $this->form->text(__('显示数量','BT_TEXTDOMAIN'))->setName('limit')->setType('number')->setSetting('default', 8);
